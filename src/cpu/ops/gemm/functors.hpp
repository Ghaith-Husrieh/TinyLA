...
struct GemmVec128Op {
    static void apply(const double* a, const double* b_packed, double* out, size_t i, size_t j, size_t k, size_t n) {
        ...
        if (k >= 2) {
            ...
            result += _mm_cvtsd_f64(sum_64);
        }

        for (; k_idx < k; k_idx++) {
            result += a[i * k + k_idx] * b_packed[j * k + k_idx];
        }
    ...
    static void apply_batch(const double* a,
        ...
        for (; k_idx < k; k_idx++) {
            result += a[a_offset + i * k + k_idx] * b_packed[b_packed_offset + j * k + k_idx];
        }

        out[c_offset + i * n + j] = result;
    }
};
